...
int part2(const std::vector<std::pair<int, std::array<std::string, 8>>> &passportList) {
  ...
  for (const auto& [flag, content] : passportList) {
    ...
    if (haveAllFieldValid(flag, content)) {
      ++validPassport;
    }
  }
  return validPassport;
...
std::vector<std::pair<int, std::array<std::string, 8>>> parseInputFile(std::string filename) {
  ...
  std::vector<std::pair<int, std::array<std::string, 8>>> passportList;
  ...
  while (getline(infile, line)) {
    ...
    if (line.empty()) {
      passportList.push_back({flag, fieldContent});
      flag = 0;
      continue;
    }
  ...
  passportList.push_back({flag, fieldContent});  // the last passport was not push by new line
  return passportList;
}


std::tuple<int, int> allInOne(std::string filename) {
  std::ifstream infile(filename);
  if (!infile.is_open()) {
    throw std::runtime_error("File Not Found : " + filename);
  }
